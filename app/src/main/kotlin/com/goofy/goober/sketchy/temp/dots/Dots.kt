package com.goofy.goober.sketchy.temp.dots
...
import com.goofy.goober.sketchy.SketchWithCache
...
@Composable
fun paint(color: Color = primarySketchColor()) = remember {
    Paint().apply {
        isAntiAlias = true
...
fun BasicGrid(modifier: Modifier = Modifier) {
    ...
    DotGrid(modifier) { u, v, x, y, time, dotCount, random ->
        drawCircle(
            color = color, radius = 17f, center = Offset(x, y)
        )
    }
}
...
fun Noise2DGrowingDots(modifier: Modifier = Modifier) {
    ...
    DotGrid(
        ...
        drawCircle(
            center = Offset(x, y), radius = radius, color = color
        )
    }
}
...
fun DotYPendulum(modifier: Modifier = Modifier) {
    ...
    DotGrid(modifier, speed = 3f) { u, v, x, y, time, dotCount, _ ->
        ...
        drawCircle(
            color = color, radius = y / size.width * 20f, center = Offset(shiftedX, y)
        )
    }
}
...
fun DotsAroundCircleWavy(modifier: Modifier = Modifier) {
    val surroundingColor = primarySketchColor()
    DotGrid(modifier) { u, v, x, y, time, _, _ ->
        ...
        val effectiveColor = if (isInCircle) color else surroundingColor
        drawCircle(
...
private fun DrawScope.drawMonotoneGrid(path: Path, color: Color) {
    drawPath(
        color = color, style = Stroke(3f), path = path
    )
}
