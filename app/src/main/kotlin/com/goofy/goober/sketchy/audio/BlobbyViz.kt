...
fun BlobbyViz() {
    ...
    val vizState = remember { VisualizerState() }
    val viz = rememberVisualizer(vizState)
...
//                    yOffset = map(sin(rad + time * 3f), -1f, 1f, 0f, 5f + (time * 10f))
                    xOffset = map(cos(rad + time * 10f), -1f, 1f, 0f, vizState.maxNoise.value)
                    yOffset = map(sin(rad + time * 3f), -1f, 1f, 0f, vizState.maxNoise.value)
