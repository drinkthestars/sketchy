...
fun Generate(modifier: Modifier = Modifier) {
    ...
    var colors by remember { mutableStateOf(Palette1.shuffled().take(2)) }
    var yAbsOffsetFactor by remember { mutableFloatStateOf(0.1f) }
    ...
    var alpha by remember { mutableFloatStateOf(0.1f) }
...
//            yAbsOffsetFactor = (yAbsOffsetFactor - 0.05f).coerceAtLeast(0.03f)
        ...
        TextureControls(
            ...
            onSaturationToggled = { saturationEnabled = it },
                ...
                .graphicsLayer {
                    if (texturingEnabled) {
                        ...
                        renderEffect = RenderEffect
                            .createRuntimeShaderEffect(NoiseGrain1, "image")
...
//                        rotation = if (i % 2 == 0) PI else 0f,
                        alpha = alpha,
                        path = path,
                        colorFilter = if (saturationEnabled) {
                            ColorFilter.colorMatrix(ColorMatrix().apply { setToSaturation(5f) })
                        } else null
                    )
                }
            }
